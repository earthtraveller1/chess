...
void window_t::show() const noexcept { glfwShowWindow(m_window); }
...
window_t::window_t()
{
    ...
    m_window =
        glfwCreateWindow(window_size, window_size, "Chess", nullptr, nullptr);
    ...
    glfwSetWindowPos(m_window, (video_mode->width - window_size) / 2,
                     (video_mode->height - window_size) / 2);
...
void window_t::glfw_error_callback(int p_error_code,
                                   const char *p_error_message) noexcept
{
    std::cerr << "[GLFW ERROR " << p_error_code << "]: " << p_error_message
              << std::endl;
}