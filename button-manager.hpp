...
class button_manager_t
{
  public:
    ...
    inline bool is_button_hovered(double pos_x, double pos_y, double width,
                                  double height)
    {
        return ((m_mouse_x > pos_x) && (m_mouse_x < (pos_x + width))) &&
               ((m_mouse_y > pos_y) && (m_mouse_y < (pos_y + height)));
    }
