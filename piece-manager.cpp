...
piece_manager_t::piece_manager_t(board_t &p_board, bool p_should_flip_on_turns)
    : m_renderer{64, "renderer-shader.vert", "piece-shader.frag"},
      m_board{p_board}, m_flipped{false},
      m_should_flip_each_turn{p_should_flip_on_turns},
      m_playing{chess::piece_t::army_e::WHITE}, m_move_checker{*this}
{
    // Load the textures.
...
void piece_manager_t::set_dragging(bool p_dragging) noexcept
{
    ...
    if (!p_dragging && !m_dragged_piece.is_empty)
    {
        ...
        if (m_move_checker.is_move_legal(m_dragged_piece, previous_position) ||
            m_move_checker.handle_castling(m_dragged_piece))
        {
            ...
            {
                m_pieces[new_piece_x][new_piece_y].role =
                    piece_t::role_e::QUEEN;
            }
            ...
            if (std::abs(m_dragged_piece.position.row -
                         previous_position.row) == 2 &&
                m_dragged_piece.role == piece_t::role_e::PAWN)
            {
                m_pieces[new_piece_x][new_piece_y].can_be_en_passant_captured =
                    true;
            }
            ...
            m_playing =
                (m_playing == piece_t::army_e::WHITE ? piece_t::army_e::BLACK
        ...
        else
        {
            ...
            m_pieces[previous_position.column][previous_position.row] =
                m_dragged_piece;
        }
    ...
    else
    {
        ...
        if ((m_pieces[dragged_piece_x][dragged_piece_y].is_empty) ||
            (m_pieces[dragged_piece_x][dragged_piece_y].army != m_playing))
        {
            return;
        }
...
void piece_manager_t::draw_piece(const piece_t &p_piece)
{
    ...
    piece.position = {static_cast<float>(p_piece.position.column),
                      static_cast<float>(p_piece.position.row)};
