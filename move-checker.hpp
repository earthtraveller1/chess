...
class move_checker_t
{
  public:
    ...
    bool is_move_legal(const piece_t &piece,
                       const piece_position_t &previous_position,
                       bool check_for_check = true);
  ...
  private:
    ...
    bool is_pawn_move_legal(const piece_t &piece,
                            const piece_position_t &previous_position);
    ...
    bool is_space_in_between_empty(const piece_position_t &a,
                                   const piece_position_t &b);

    // Check if a specific position is attacked by any enemy pieces
    bool is_space_attacked(const piece_position_t &position,
                           piece_t::army_e enemy_army);
};
} // namespace chess
