...
class application_t
{
  public:
    ...
    inline static void mouse_button_callback(GLFWwindow *window, int button,
                                             int action, int)
    {
        reinterpret_cast<application_t *>(glfwGetWindowUserPointer(window))
            ->m_scene_manager.active_on_mouse_click(button, action);
    }
    ...
    {
        reinterpret_cast<application_t *>(glfwGetWindowUserPointer(window))
            ->m_scene_manager.active_on_mouse_move(xpos, ypos);
    }

    inline static void key_callback(GLFWwindow *window, int key, int,
                                    int action, int)
    {
        reinterpret_cast<application_t *>(glfwGetWindowUserPointer(window))
            ->m_scene_manager.active_on_key_event(key, action);
    }
