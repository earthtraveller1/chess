...
game_scene_t::game_scene_t(scene_manager_t &p_scene_manager,
                           uint16_t p_window_width, uint16_t p_window_height)
    : m_board(), m_scene_manager(p_scene_manager),
      m_piece_manager(
          m_board,
          option_loader_t("options.txt").get_option_value("FlipBoardOnMove") ==
              "true"),
      m_renderer(8, "renderer-shader.vert", "renderer-shader.frag"),
      m_is_paused(false), m_window_width(p_window_width),
...
void game_scene_t::render()
{
    ...
    if (m_is_paused)
    {
        ...
        if (m_button_manager.is_button_hovered(
                resume_button.position.x, resume_button.position.y,
                resume_button.size.x, resume_button.size.y))
        {
        ...
        if (m_button_manager.is_button_hovered(
                restart_button.position.x, restart_button.position.y,
                restart_button.size.x, restart_button.size.y))
        {
...
void game_scene_t::on_mouse_click(int p_button, int p_action)
{
    ...
    else
    {
        ...
        {
            if (m_button_manager.is_button_hovered((8.0 - 3.0) / 2.0, 3.0, 3.0,
                                                   3.0 / 4.0))
            {
                m_is_paused = false;
            }

            if (m_button_manager.is_button_hovered((8.0 - 3.0) / 2.0, 4.0, 3.0,
                                                   3.0 / 4.0))
            {
                m_scene_manager.set_active(std::make_shared<game_scene_t>(
                    m_scene_manager, m_window_width, m_window_height));
            }
        }
    }
}
...
void game_scene_t::on_mouse_move(double xpos, double ypos)
{
    ...
    m_button_manager.update_mouse_positions((xpos / m_window_width) * 8.0,
                                            (ypos / m_window_height) * 8.0);
}
